...
class Template {
	...
	{	
		...
		$output = str_replace('{yield}', $this->layout_yield(true), $output);
	 ...
	 * @return void
	 */
	public function layout_yield($bypass=false) 
	{ 
    ...
    public function getFlashes()
    {
        $flashes = array();
        $ci = get_instance();
        $flashContainer = $ci->input->getRequest()->getSession()->getFlashBag();
        if (!empty($flashContainer)) {
            foreach ($flashContainer as $type => $messages) {
                if (!empty($type)) {
