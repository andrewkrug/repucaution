...
class Kohana_Arr {
    ...
    /**
     * Parse $array and remove values if their keys doesn't exist in $keys
     *
     *      $array1 = array( 'name' => 'alex', 'email' => 'test@test.com' , 'company' => 'Ikantam', 'position' => 'SEO' );
     *
     *      Arr::remove( $array1, array('name', 'position')   );
     *
     *      // Output will be:
     *      array('name' => 'alex', 'position' => 'SEO');
     *
     * @param array | $array
     * @param array | $keys
     *
    ...
    public static function collect($array) {
        ...
        foreach ($array as $key => $subarr) {
            foreach ($subarr as $subkey => $subvalue) {
                if ( ! isset($new_array[$subkey])) {
                    $new_array[$subkey] = array();
                }
                $new_array[$subkey][$key] = $subvalue;
            }
        }
        return $new_array;
    }
    ...
    /**
     * Retrieve value from array using key and validate it using $validate function,
     * if no value found or if value fails validation return default value.
     * Validation considered passed if $validate function returns true.
     *
     * @param array $array
     ...
     * @return mixed
     */
    public static function getValidate(array $array, $key, $default, \Closure $validate)
    {
